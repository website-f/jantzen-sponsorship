...
const dropzoneInit = () => {
  ...
  const Selector = {
    ...
    HIDDEN_INPUT: 'input[name="file_names"]'
  };
  ...
  !!dropzones.length &&
    dropzones.forEach(item => {
      ...
      const options = Object.assign(
        {
          url: '/upload', // Update this to your file upload URL
          addRemoveLinks: true,
          ...
          init: function init() {
            ...
            thisDropzone.hiddenInput = document.querySelector(Selector.HIDDEN_INPUT);
            thisDropzone.fileData = [];
            ...
            thisDropzone.on(Events.ADDED_FILE, function addedfile(file) {
              ...
              reader.onload = (event) => {
                ...
                thisDropzone.fileData.push(fileData);
                thisDropzone.hiddenInput.value = JSON.stringify(thisDropzone.fileData);
              };
              reader.readAsDataURL(file);
            });

            thisDropzone.on(Events.REMOVED_FILE, function removedfile(file) {
              thisDropzone.fileData = thisDropzone.fileData.filter(f => f.name !== file.name);
              thisDropzone.hiddenInput.value = JSON.stringify(thisDropzone.fileData);
            });

            thisDropzone.on(Events.COMPLETE, () => {
